...
function QuickActions() {
  return (
    <ClientOnly fallback={<QuickActionsSkeleton />}>
      <QuickActionsContent />
    </ClientOnly>
  )
}
...
function RecentActivity() {
  return (
    <ClientOnly fallback={<RecentActivitySkeleton />}>
          ...
          {activities.length > 0 ? (
            activities.slice(0, 5).map((activity) => (
              <div key={activity.id} className="flex items-start space-x-3">
                <div className={cn('w-2 h-2 rounded-full mt-2 flex-shrink-0', getActivityIcon(activity.type))} />
        ...
        <div className="space-y-6">
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="agents" className="space-y-4">
              <ClientOnly fallback={<AgentsSkeleton />}>
                <AgentsSection />
              </ClientOnly>
            </TabsContent>

            <TabsContent value="workflows" className="space-y-4">
              <ClientOnly fallback={<WorkflowsSkeleton />}>
                <WorkflowsSection />
              </ClientOnly>
            </TabsContent>
...
// Individual sections wrapped in ClientOnly
function DashboardActions() {
  const { refreshData } = useDashboard()
  const { isApiConnected } = useConnectionStatus()
  const { isLoading } = useDashboardUI()
